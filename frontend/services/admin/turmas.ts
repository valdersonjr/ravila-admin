...
export const turmasService = {
  ...
  criar: (data: { nome: string; nivel_id?: number; professor_id: number }) => apiAuth.post<Turma>("/turmas/", data),
  atualizar: (id: number, data: { nome?: string; nivel_id?: number; professor_id?: number; status?: string }) => apiAuth.put<Turma>(`/turmas/${id}`, data),
